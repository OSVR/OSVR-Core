...
namespace util {
    ...
    /// The real resolution of the clock: 100 nanoseconds, assumes the default
...
//
    ...
    class TimeValueClock {
      public:
        /// Custom rep, which will undoubtedly cause issues that we'll manually
        /// resolve later.
        ...
        /// When we want a short-ish duration (time difference), we can go to a
        ...
        template <typename Derived> class TimeValueChronoWrapperBase {
          public:
            ...
            getMicrosecondsPartAsDuration() const {
                return time_value_microseconds_duration{getMicrosecondsPart()};
            }

            derived_type const &derived() const {
                return *static_cast<derived_type const *>(this);
            }
            util::time::TimeValue const &get() const { return derived().get(); }
        };
    } // namespace detail
    ...
    class TimeValueChronoValueWrapper
      ...
      public:
        explicit TimeValueChronoValueWrapper(util::time::TimeValue const &tv)
            : m_timeval(tv){};
    ...
    class TimeValueChronoConstRefWrapper
      ...
      public:
        explicit TimeValueChronoConstRefWrapper(util::time::TimeValue const &tv)
            : m_timeval(tv){};
        /// noncopyable
        TimeValueChronoConstRefWrapper(TimeValueChronoConstRefWrapper const &) =
            delete;
        /// nonassignable
        TimeValueChronoConstRefWrapper &
    ...
    /// Function that forwards a - b, where a is a wrapped TimeValue while b
    /// isn't wrapped, to the main implementation above.
    template <typename Derived>
    inline time_value_scalar_duration
    operator-(detail::TimeValueChronoWrapperBase<Derived> const &a,
              util::time::TimeValue const &b) {
        return a - TimeValueChronoConstRefWrapper{b};
    }
    /// Function that forwards a - b, where a isn't wrapped while b is a wrapped
    /// TimeValue, to the main implementation above.
    template <typename Derived>
    inline time_value_scalar_duration
    operator-(util::time::TimeValue const &a,
              detail::TimeValueChronoWrapperBase<Derived> const &b) {
        return TimeValueChronoConstRefWrapper{a} - b;
    }
    ...
    operator-(TimeValueClock::time_point const &a,
        ...
        using namespace std::chrono;
        auto aRaw = a.time_since_epoch().count();
        auto bRaw = b.time_since_epoch().count();
        auto diffSeconds =
            TimeValueClock::seconds_duration(aRaw.seconds - bRaw.seconds);
        auto diffUsec = TimeValueClock::microseconds_duration(
            aRaw.microseconds - bRaw.microseconds);
        return duration_cast<time_value_scalar_duration>(diffSeconds) +
               duration_cast<time_value_scalar_duration>(diffUsec);
    }

    /// Treat/convert a TimeValue into a time point
    inline TimeValueClock::time_point as_time_point(time::TimeValue const &tv) {
    ...
    to_duration(time::TimeValue const &tv) {
        using namespace std::chrono;
        return duration_cast<TimeValueClock::scalar_duration>(
                   TimeValueClock::seconds_duration{tv.seconds}) +
               duration_cast<TimeValueClock::scalar_duration>(
                   TimeValueClock::microseconds_duration{tv.microseconds});
    }
#endif
} // namespace util
} // namespace osvr
