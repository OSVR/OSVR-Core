...
public:
    ...
    /// @param applicationIdentifier A string identifying your application. Reverse DNS format strongly suggested.
    /// @param flags initialization options (reserved) - pass 0 for now.
    ClientContext(const std::string& applicationIdentifier, uint32_t flags = 0u);

    /// @brief Initialize the context with an existing context.
    /// @note The ClientContext class takes ownership of the OSVR_ClientContext pointer.
    ...
    /// @param[out] iface The interface object. May be freed when no longer needed, otherwise it will be freed when the context is closed.
...
inline ClientContext::ClientContext(const std::string& applicationIdentifier, uint32_t flags)
{
    m_context = osvrClientInit(applicationIdentifier.data(), flags);
}

inline ClientContext::ClientContext(OSVR_ClientContext context) : m_context(context)
{
    // do nothing
}
...
inline InterfacePtr ClientContext::getInterface(const std::string& path)
{
    ...
    OSVR_ReturnCode ret = osvrClientGetInterface(m_context, path.c_str(), &interface);
    if (OSVR_RETURN_SUCCESS != ret) {
        throw std::runtime_error("Couldn't create interface because the path was invalid.");
    }
...
inline std::string ClientContext::getStringParameter(const std::string& path)
{
    size_t length = 0;
    OSVR_ReturnCode ret = osvrClientGetStringParameterLength(m_context, path.c_str(), &length);
    if (OSVR_RETURN_SUCCESS != ret) {
        throw std::runtime_error("Invalid context or null reference to length variable.");
    }
    ...
    if (OSVR_RETURN_SUCCESS != ret) {
        throw std::runtime_error("Invalid context, null reference to buffer, or buffer is too small.");
    }
