...
/** @brief Class representing a host and resource path in the OGVR logical
   system.
    ...
    Instances of this class always refer to an absolute path, with host and full
   ...
   pseudo-entries (for current directory and parent directory) is not supported
...
class ResourcePath {
  public:
    ...
    /// @brief Go to the named child. The name may include slashes for multiple
    /// levels.
    ...
    /// @brief Gets the path following the hostname as a string, starting with a
    /// leading slash.
