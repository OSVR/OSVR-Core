...
class AsyncAccessControl : boost::noncopyable {
  public:
    ...
    /// @name
    /// @brief Check for waiting async thread, and give it permission to send if
    /// found.
    ///
    ...
    /// @brief Check for waiting async thread, and deny it permission to send if
    /// found.
    ///
  ...
  private:
    /// @brief Messages/status that may be set by the main thread for read by
    /// the async thread.
    ...
    /// @brief mutex keeping the main thread from running away before the async
    /// is done.
    boost::mutex m_mutDone;

    /// @brief For the main thread sleep/wake awaiting completion of the async
    /// thread's work.
    boost::condition_variable m_condMainThread;
    /// @brief for the async thread's sleep/wake awaiting response from the main
    /// thread.
...
/// @brief RAII object for use by an async thread to manage a request to send
///
/// @todo allow timeout on RTS
/// `enum RequestResults { RR_CTS_GRANTED, RR_DENIED, RR_TIMED_OUT };`
class RequestToSend : boost::noncopyable {
  public:
    /// @brief Creates an RAII object to manage the request to send - note that
